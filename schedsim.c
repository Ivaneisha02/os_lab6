...
{ 
  ...
  while (complete != n) {
    shortest -=1;
    for (i=0; i<n; i++) {
      if ((plist[i].art <= t) && (bt_remaining[i] < min_bt) && (bt_remaining[i] > 0)) {
        min_bt = bt_remaining[i];
        shortest = i;
      }
    }
    bt_remaining[shortest] = 0;
    complete++;
    finish_time = t+1;
...
{ 
    ...
    */
    ...
    while (complete != n) {
        ...
        for (int i = 0; i < n; i++) {
            if (plist[i].art <= t && bt_remaining[i] > 0) {
                if (highest_priority == -1 || plist[i].pri < plist[highest_priority].pri) {
                    highest_priority = i;
                }
            }
        }
        ...
        bt_remaining[highest_priority] = 0; 
        complete++;

        finish_time = t + plist[highest_priority].bt;
        plist[highest_priority].wt = finish_time - plist[highest_priority].art - plist[highest_priority].bt;

        if (plist[highest_priority].wt < 0) {
            plist[highest_priority].wt = 0;
        }
